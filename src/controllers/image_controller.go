...
func (f *ImageController) Resize() {
	...
	fileUrl := file_storage.GetPrivateFileUrl(file.Id)
...
// @Description 图片模糊
// @Param   fid    path    int  true    "文件id"
// @Param   sigma  path    float  true    "模糊度"
// @Success 200  {object} libs.FileNode
// @router /blur [get]
func (f *ImageController) Blur() {
	fileId, _ := f.GetInt64("fid")
	sigma, _ := f.GetFloat("sigma")
	if fileId <= 0 {
		f.Json(libs.NewError("img_blur_fileid", "F6021", "fileid don't less equal zero", ""))
		return
	}
	if sigma <= 0 {
		f.Json(libs.NewError("img_blur_sigma", "F6022", "sigma don't less equal zero", ""))
		return
	}
	file := file_storage.GetFile(fileId)
	if file == nil {
		f.Json(libs.NewError("img_blur_file_notexist", "F6023", "file not exist", ""))
		return
	}
	if file.IsDeleted {
		f.Json(libs.NewError("img_blur_file_deleted", "F6024", "file deleted", ""))
		return
	}
	if file.ExtName == "gif" {
		f.Json(libs.NewError("img_blur_gif_notsupport", "F6025", "gif photo not support resize", ""))
		return
	}
	if !utils.IsImage(file.ExtName) {
		f.Json(libs.NewError("img_blur_file_isnot_image", "F6025", "file isn't image file", ""))
		return
	}
	fileUrl := file_storage.GetPrivateFileUrl(file.Id)
	request := httplib.Get(fileUrl)
	...
	data, err := request.Bytes()
	if err != nil {
		f.Json(libs.NewError("img_blur_fileread_fail", "F6026", err.Error(), ""))
		return
	}
	...
	srcImg, _, err := image.Decode(buf)
	if err != nil {
		f.Json(libs.NewError("img_blur_img_decode_fail", "F6029", err.Error(), ""))
		return
	}
	...
	resizeName := fmt.Sprintf("%s-blur-%f.%s", utils.FileName(file.OriginalName), sigma, file.ExtName)
	node, err := file_storage.SaveFile(fileData, resizeName, file.Id)
	if err != nil {
		f.Json(libs.NewError("img_blur_saveFile_fail", "F6028", err.Error(), ""))
		return
	}
	f.Json(node)
}
