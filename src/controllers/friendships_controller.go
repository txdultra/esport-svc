...
func (c *FriendShipsController) URLMapping() {
	...
	c.Mapping("BothFriends", c.BothFriends)
...
func (c *FriendShipsController) Followers() {
	...
	if uid <= 0 || current_uid == uid {
		fship.ResetEventCount(current_uid)
	}
...
// @Title 用户互粉列表
// @Description 用户互粉列表(拼音首字母列表)
// @Param   access_token   path   string  true  "access_token"
// @Success 200
// @router /both_friends [get]
func (c *FriendShipsController) BothFriends() {
	current_uid := c.CurrentUid()
	fs := &passport.FriendShips{}
	pymaps := fs.BothFriendUidsPy(current_uid)
	...
	for _, v := range py_chars {
		...
		if ok {
			out_members := []*outobjs.OutMember{}
			for _, _uid := range vs {
				out_member := outobjs.GetOutMember(_uid, 0) //无须带出关系
				if out_member != nil {
					out_members = append(out_members, out_member)
				}
			}
...
// @Param   certified   path   int  false  "是否认证会员"
// @Success 200
// @router /recmds [get]
func (c *FriendShipsController) Recmds() {
	uid := c.CurrentUid()
	nums, _ := c.GetInt("nums")
	certified, err := c.GetBool("certified")
	if nums <= 0 {
		nums = 8
	}
	if err != nil { //未传参数
	...
	nums += 1
	fship := &passport.FriendShips{}
	uids := fship.RecmdFriendUids(uid, nums, certified, false)
	outs := []*outobjs.OutMember{}
	//电竞圈放第一位 start
	friends := fship.FriendIds(uid)
	had := false
	...
	if !had {
		out_m := outobjs.GetOutMember(21, uid)
		if out_m != nil {
