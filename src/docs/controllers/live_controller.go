...
func (c *LiveController) PerGets() {
	...
	//}
	//2分钟间隔缓存
	query_cache_key := fmt.Sprintf("front_fast_cache.personallive.query:words_%s_gameids_%s_p_%d_s_%d_mode_%s_uid_%d_onlyc_%t",
		query, game_ids, page, size, match_mode, uid, only_c)
...
func (c *LiveController) SubsReminded() {
	...
	//清空计数器
	noticer.ResetMemberNoticeCount(uid)
	c.Json(out_spms)
}
