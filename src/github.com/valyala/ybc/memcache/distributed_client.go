...
func (c *DistributedClient) init(isDynamic bool) {
	...
	c.clientsIdxMap = make(map[int]*Client)
...
func (c *DistributedClient) registerClient(client *Client) bool {
	...
	idx := 0
	if len(c.clientsIdxMap) > 0 {
		for k, _ := range c.clientsIdxMap {
			if k > idx {
				idx = k
			}
		}
	...
	c.clientsIdxMap[idx] = client
	//clientIdx := len(c.clientsList) - 1
	c.clientsHash.Add([]byte(serverAddr), idx)
	return true
}

var communicationFailureEventHandleRunTimer *time.Timer
...
func (c *DistributedClient) addServer(serverAddr string) {
	...
	//失败重新启动连接
	client.RegisterCommunicationFailureEventHandler(func(addr string) {
		c.cmtx.Lock()
		defer c.cmtx.Unlock()
		if communicationFailureEventHandleRunTimer == nil {
			c.DeleteServer(addr)
			fmt.Println("memcache:" + addr + " communication failure,30 seconds restart")
			communicationFailureEventHandleRunTimer = time.AfterFunc(30*time.Second, func() {
				c.AddServer(addr)
				communicationFailureEventHandleRunTimer = nil
			})
		}
	})
...
func (c *DistributedClient) clientNolock(key []byte) *Client {
	...
	//fmt.Println(clientIdx)
	if client, ok := c.clientsIdxMap[clientIdx]; ok {
		return client
	}
	panic("not exist client index")
