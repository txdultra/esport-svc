...
type Client struct {
	...
	communicationFailureEventHandler func(string)
}
...
func (c *Client) RegisterCommunicationFailureEventHandler(handler func(string)) {
	c.communicationFailureEventHandler = handler
}
...
func (c *Client) do(t tasker) (err error) {
	...
	if !t.Wait() {
		...
		go c.communicationFailureEventHandler(c.ServerAddr)
	}
	return
}
